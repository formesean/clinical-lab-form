...
export async function POST(req: Request) {
  ...
  if (formType === "OBT") {
    sheet.getCell(`F27`).value = v(raw.color);
    sheet.getCell(`D28`).value = v(raw.consistency);
    sheet.getCell(`F31`).value = v(raw.hhma);
    sheet.getCell("C43").value = v(raw.remarks);
    sheet.getCell("A47").value = v(raw.performed);
  }
