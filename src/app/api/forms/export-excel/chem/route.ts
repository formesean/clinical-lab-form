...
function getTemplateFilename(sex: Sex, unit: Unit): string {
  const sexLetter = sex === "MALE" ? "M" : "F";
  const unitLabel = unit === "si" ? "SI" : "Conv";
  return `CHEM_${sexLetter}_${unitLabel}.xlsm`;
}

function getSheetName(sex: Sex, unit: Unit): string {
  const sexLetter = sex === "MALE" ? "M" : "F";
  const unitLabel = unit === "si" ? "SI" : "Conv.";
...
export async function POST(req: Request) {
  ...
  const sheetName = getSheetName(sex as Sex, unit as Unit);
...
function updateDefinedNames(workbookXml: string, sheet: WorkbookSheet): string {
  ...
  const kept: string[] = [];
  while ((defMatch = defRegex.exec(inner))) {
...
function parseRelationships(relXml: string): RelationshipInfo[] {
  ...
  while ((match = relRegex.exec(relXml))) {
    const raw = match[0];
    const attrs = parseXmlAttributes(raw);
...
function filterContentTypes(
  ...
  while ((match = overrideRegex.exec(contentTypesXml))) {
    const raw = match[0];
    const attrs = parseXmlAttributes(raw);
...
function extractSharedStringText(sharedStringXml: string): string {
  ...
  let match: RegExpExecArray | null;
  let text = "";
  while ((match = tRegex.exec(sharedStringXml))) {
